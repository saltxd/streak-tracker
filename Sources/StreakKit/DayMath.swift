...
public enum DayMath {
    ...
    /// The displayed streak. The anchor day counts as **1**; floored at 0 so a clock
    /// set backwards (or a freshly-reset anchor in the future) never shows a negative.
    public static func streakValue(startDay: Date?, now: Date, calendar: Calendar = .current) -> Int {
        guard let startDay else { return 0 }
        return max(0, dayCount(from: startDay, to: now, calendar: calendar) + 1)
    }

    /// Anchor used when *starting* a streak — today reads 1.
    public static func today(_ now: Date, calendar: Calendar = .current) -> Date {
        calendar.startOfDay(for: now)
    }

    /// Anchor used when *resetting* — today reads 0, tomorrow reads 1.
    public static func tomorrow(_ now: Date, calendar: Calendar = .current) -> Date {
